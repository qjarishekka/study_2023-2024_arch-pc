...
#Задание 1 - составить формулу formula1b
def formula1b():
    # Predicates to use:
...
# Sentence: if I'm hungry and have money, I'm going to buy KFC
...
def example1():
    ...
    money = Atom('Money')
    kfc = Atom('KFC')
    return Implies(And(hungry,money), kfc)
...
# Return a formula which defines Daughter in terms of Female and Child (FemaleChild).
# every daughter is a child and a female
...
# every sibling has a sibling
def example2():
    def sibling(x,y): return Atom('Sibling', x,y) #whether x is y's sibling

    return  Forall('$x',Exists('$y',sibling('$x','$y')))
