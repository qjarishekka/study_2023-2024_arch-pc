...
def triangle_FS(U, a=None, b=None, c=None, d=None, h=1, form='equal'):
    if form in ['equal', 'greater', 'less']:
        ...
        if c < d:
            for el in U[(U >= c) * (U <= d)]:
                r = 0. if form == 'less' else h * float(el - c) / (d - c) if form == 'greater' else h * float(
                    d - el) / (d - c)
                z = Mu.get(el, 0)
                Mu[el] = r if r > z else z
        return Mu
...
def FS_plot(FS, labels=None, title=None, name=None):
    for idx, el in enumerate(FS):
        mas = np.array(sorted(el.items(), key=lambda x: x[0]), dtype=np.float64).T
        # print(idx, mas[0], mas[1])
        if labels is None:
            lab = ' '
        else:
            lab = labels[idx]
...
def FS_describe(FS, method='minmax', verbose='True'):
    ...
    if verbose:
        ...
        print(f'Максимумы:', (Fst_max, Cntr_max, Lst_max))
...
def FS_quantificator(FS, quantificators=['очень']):
    mas = np.array(list(FS.items())).T
...
quants = [['не'], ['очень'], ['наверное']]
labels = ['не', 'очень', 'наверное']
#FS_plot([FS_quantificator(Pr1, quantificators=x) for x in quants], labels=labels)

quants = [['не', 'очень'], ['очень', 'не'], ['наверное', 'не', 'очень']]
labels = ['не очень', 'очень не', 'наверное не очень']
#FS_plot([FS_quantificator(Pr1, quantificators=x) for x in quants], labels=labels)
...
def FS_divorse(FSs, Ps=None, method='minmax'):
    U = set()
    for FS in FSs:
        U = set.union(U, FS.keys())
    res = dict()
...
# FS_plot([lhc_Giri], title='тяжелая + легкая гири(с очисткой)')


Eng_words = np.arange(0, 6000, 100)
